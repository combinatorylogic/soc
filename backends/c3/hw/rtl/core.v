...
`ifndef CPUNAME
 `define CPUPREFIX sys
 `define CPUNAME cpuc3
`endif
...
// A core based on C2, but with caches and interrupts (i.e., watch out for long running extensions - they're better left for minion cores).
//
// Both IC and DC misses can stall the entire pipeline

//
// When IC is ready but not acked (ack is coming next cycle anyway), we unstall the pipeline and inject a bubble.
...
module `CPUNAME 
          (input clk,
           ...
           // For the port A we do not have an expclicit REQ signal, assuming that we want a value every cycle, ideally.
           // Port A is an instruction cache, no more direct access
           input [31:0]  ram_data_in_a,
           output [31:0] ram_addr_in_a,
           input         ram_data_ack_a, 
           input         ram_ready_a, 

           // Port B is a data cache
           input [31:0]  ram_data_in_b,
           output [31:0] ram_addr_in_b,
           output [31:0] ram_data_out_b,
           output        ram_we_out,
           output        ram_re_out, // it stays asserted until acked
           input         ram_data_ack_b, // Writes are not ACKed, only reads; Writes do not stall the pipeline either, obviously
           input         ram_ready_b, 
   ...
   parameter IRQ_HANDLERS_BASE = 4; // with 0 being an initial PC value on reset
   ...
   assign stall = stall_cpu | stall_exec | stall_mem | ~ram_data_ready_a; // ?!?
...
`endif
   ...
   reg            doing_irq;  // acknowledged an IRQ
   reg [31:0]     irq_saved_PC;


   // IRQ vectors array must be populated with simple jump instructions
   //  followed by nops.
   //
   // An IRQ handler must save all the dirtied registers in a dedicated 
   // location (no stack! no nested IRQs!)
   //
   // An important note about IRQs: must also somehow save an R0 destination forwarding info in
   // case if the next instruction executed after IRET is a SELECT.
   //
   // Or, for now, just do not ACK if we have a TypeA with a DST=0 in DECODE0 stage.
   assign fetch_PC_irq_next = IRQ_HANDLERS_BASE + {irq_num, 2'b0};
   // Can do an IRQ iff there are no branch instructions
   //   in DECODE0, DECODE, EXEC stages
   wire [31:0]    decode0_Instr; // Instruction as of beginning of DECODE0
   wire           decode0_typea;
   reg [31:0]     decode_Instr;
   reg [31:0]     exec_Instr;

   wire no_imminent_branch_insns =
        ~(decode0_Instr[0] | decode_Instr[0] | exec_Instr[0]);

   wire no_dst0_op = ~(decode0_typea & decode0_Instr[11:7] == 0);
   
   wire           can_do_irq = irq & no_imminent_branch_insns & no_dst0_op & ~doing_irq;
   // also, we ack an IRQ if not stalled and can_do_irq

   wire [31:0] fetch_PC_next_inner =(
                         decode_ready?
   ...
   assign fetch_PC_next =
                         can_do_irq?fetch_PC_irq_next:fetch_PC_next_inner;
   ...
   always @(posedge clk)
     if (~rst) begin
        ...
        decode_ready <= 1;
        unstall <= stall;
        irq_ack <= can_do_irq; // i.e., only asserted for 1 cycle
        irq_saved_PC <= can_do_irq?fetch_PC_next_inner:irq_saved_PC;
        // TODO: doing_irq logic - how we're supposed to return?
     end
   ...
   wire [31:0]    ram_data_in_ax;
   assign ram_data_in_ax = ram_ack_a?ram_data_in_a:NOP_INSTR; // blow bubbles
   ...
   wire           decode_doing_irq_next;

   localparam IRET_INSTR= 2; // 01000000...
   
   
   assign decode_doing_irq_next =
                                 doing_irq?~(decode0_Instr == IRET_INSTR):can_do_irq;
   ...
   always @(posedge clk)
     if (~rst) begin
        ...
        // We have to set it here, at a stage where IRET instruction is decoded
        doing_irq <= decode_doing_irq_next;
...
`ifndef DISABLE_MICROOPS
   ...
   assign is_muop = (~executing_microops) & (ram_data_in_ax[2:0] == 1);
   assign is_ret = ram_data_in_ax[31:3] == 0;
        ...
        case (muop_state)
          S_MU_IDLE: begin
             if (is_muop) begin // start muops
                muop_call_dst <= ram_data_in_ax[31:3];
                if (is_call) begin
...
`endif
   ...
   assign ram_re_out = exec_ram_re_out;
   assign stall_mem = exec_ram_re_out & ~ram_ack_b;
   ...
   assign exec_ram_re_next = exec_typem&(~exec_isstore);
...
`endif
        ...
        exec_ram_re_out <= exec_ram_re_out_next; // if the next instruction is 
                                                 // also a load, REQ stays even 
                                                 // after an ACK
...
`endif
   ...
   // sorry for a bit of a misnomer here - "mem" is a register file, actually
