...
inline verilog exec(pos, x) {
   ...
   w3_off_r <= w3_off;
   w3_L_r <= w3_L;
   w3_addr_r <= w3_pos + {w3_L[28:0],3'b0};
};
...
//   transactions).
{
		...
		reg ram1_cache_hit;
                ...
                assign ram1_burstcount = ram1_burstcount_r;
	...
	inline verilog exec(dst) {
	  if (ram1_prev_address == {3'b0, dst[28:3]}) begin
  	    ram1_cache_hit <= 1;
	    ram1_read_r <= 0;
	  end else begin
	    ram1_address_r <=    {3'b0, dst[28:3]};
	    ram1_read_r <= 1;
	    ram1_cache_hit <= 0;
	    ram1_prev_address <= {3'b0, dst[28:3]};
	  end
	} wait (ram1_cache_hit | ram1_readdatavalid) {
	  ram1_value <= ram1_cache_hit?ram1_value:ram1_readdata;
          ram1_read_r <= 0;
	} else {
...
inline void _ram1_slow_write_1(uint32 dst) // never reads
{
        inline verilog define {
	  	reg [63:0] ram1_value_1;
	  	reg [63:0] ram1_writedata_1;
		reg [28:0] ram1_prev_address_1;
        };
	
        inline verilog reset {
	  ram1_value_1 <= 0;
	  ram1_writedata_1 <= 0;
	  ram1_prev_address_1 <= 0;
        };

        inline verilog exec(dst) {
           if (ram1_prev_address != {3'b0, dst[28:3]}) begin // flush if address changed
	      ram1_value_1 <= ram1_value;
 	      ram1_prev_address_1 <= ram1_prev_address;
	      ram1_prev_address <= {3'b0, dst[28:3]};
	      ram1_address_r <= ram1_prev_address;
	      ram1_writedata_r <= ram1_value;
	      ram1_writedata_1 <= ram1_writedata_r;
	      ram1_write_r <= 1;
	   end
	} wait (~ram1_write_r || (~ram1_waitrequest & ram1_write_r)) {
          ram1_writedata_r <= ram1_writedata_1;
	  ram1_write_r <= 0;
	};

	inline verilog exec(dst) {
	  if (ram1_prev_address == {3'b0, dst[28:3]}) begin
 	    ram1_value <= ram1_value;
	  // end else if (ram1_prev_address_1 == {3'b0, dst[28:3]}) begin
	  // ram1_value <= ram1_value_1;
          end else ram1_value <= 0;					   
	};

        inline verilog exec(dst) {
 	   ram1_value <= (ram1_value & (~ram1_bytes_mask)) | ram1_writedata_r;
	};
}


inline void _ram1_cls() {
  inline verilog define { reg [15:0] burstctr; };
  inline verilog reset { burstctr <= 0; };
  int32 ctr = 0;
  for (int32 dst = inline verilog exec {}
                      return ( 
			      {6'b10, vga_bufid[1:0], 20'd0}
			);
       ctr < 48600; ctr++, dst += 16) {
  inline verilog exec(dst) {
    ram1_address_r <= dst;
    ram1_writedata_r <= 0;
    ram1_write_r <= 1;
    ram1_burstcount_r <= 16;
    burstctr <= 0;
  } wait (burstctr == 15) {
    ram1_write_r <= 0;
    burstctr <= 0;
...
inline void _ram1_w3b_1(int32 dst, int32 v3b)
{
        int32 off = inline verilog  exec(dst) {} return (w3_off_r);
        if (off > 5) { // 2-word write
                int32 addr = dst;
                int32 off1 = off==7?8:9;
                _ram1_calc_mask(off1, v3b); 
                _ram1_slow_write_1(addr-8);
		
                _ram1_calc_mask(off, v3b);
                _ram1_slow_write_1(addr);
        } else {
                int32 addr = dst;
                _ram1_calc_mask(off, v3b);
                _ram1_slow_write_1(addr);
...
inline void _vga_putpixel1(int32 rowpos, int32 x, int32 color)
{
	_ram1_calc_pixeloffsets(rowpos, x);
        int32 addr = inline verilog exec (x) {}
                      return ( 
